...
const {sendmail,approvelmail} = require('../middleware/nodemail');
...
const startListening = () => {
  amqp.connect(RABBITMQ, (err, conn) => {
    ...
    conn.createChannel((err, ch) => {
      ...
      if(queuess){
        ch.assertQueue(queuess, { durable: false });
      ch.consume(queuess, async (msg) => {
        if (msg !== null) {
          const notificationInfo = JSON.parse(msg.content.toString());
          console.log(notificationInfo)
          approvelmail(notificationInfo.email,notificationInfo.user_id,notificationInfo.comapnyname,notificationInfo.companylocation,notificationInfo.jobTitle)
          ch.ack(msg);}})
      }
    });
  });
